...
export function MainLayout({ children }) {
  ...
  const roleLabel =
    { USER_ROLE: 'Cliente', EMPLOYEE_ROLE: 'Empleado', ADMIN_ROLE: 'Administrador' }[role] || role;
  ...
  const userFooter = (
    <div className='flex items-center justify-between gap-3'>
      ...
      <div className='flex gap-2'>
        <Button variant='ghost' size='icon' onClick={() => setProfileOpen(true)} title='Ver perfil'>
    ...
    <div className='flex min-h-screen bg-background text-foreground'>
      <aside className='hidden w-64 flex-col border-r border-border bg-card md:flex'>
        ...
        <nav className='flex-1 space-y-0.5 px-4 overflow-y-auto'>
          {navItems.map((item, i) => (
            <div key={item.href || `divider-${i}`}>{renderNavItem(item)}</div>
          ))}
        </nav>
          ...
          <div className='fixed inset-0 z-50 md:hidden'>
            ...
            <aside className='absolute inset-y-0 left-0 flex w-72 max-w-[85vw] flex-col border-r border-border bg-card shadow-xl'>
              ...
              <nav className='flex-1 space-y-0.5 px-4 py-4 overflow-y-auto'>
                {navItems.map((item, i) => (
                  <div key={item.href || `divider-${i}`}>{renderNavItem(item, true)}</div>
                ))}
              </nav>

              <div className='border-t border-border p-4'>{userFooter}</div>
            </aside>
          </div>
        )}
