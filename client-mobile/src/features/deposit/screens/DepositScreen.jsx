...
import { getUserAccounts, depositToAccount } from '../../../shared/api/bankClient';
...
const DepositScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAmount('');
      setDescription('');
      Alert.alert('Éxito', `Depósito de ${formatMoney(numAmount, selectedAccount.currency)} exitoso`);
      
      setTimeout(() => {
        setSuccess(false);
      }, 3000);
    } catch (err) {
      Alert.alert('Error', err.response?.data?.message || err.message || 'No se pudo procesar el depósito');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        <View style={styles.header}>
          <MaterialIcons name="arrow-downward" size={28} color={theme.colors.success} />
          <View style={styles.headerText}>
            <Text style={styles.title}>Depósito</Text>
            <Text style={styles.subtitle}>Acredita fondos en una cuenta activa</Text>
          </View>
        </View>
        ...
        <Card padding="lg" style={styles.card}>
          {success && (
            <View style={styles.successBanner}>
              ...
              <Text style={styles.successText}>Depósito exitoso</Text>
            </View>
          )}
          ...
          {selectedAccount && (
            <View style={styles.accountInfo}>
              <View style={styles.infoItem}>
                ...
                <Text style={styles.infoValue}>{formatMoney(selectedAccount.balance, selectedAccount.currency)}</Text>
              </View>
            </View>
          )}

          <View style={styles.formGroup}>
            <Text style={styles.label}>Montos rápidos {selectedAccount ? `(${selectedAccount.currency})` : ''}</Text>
            <View style={styles.quickAmounts}>
              {QUICK_AMOUNTS.map((q) => (
                <Button
                  key={q}
                  title={selectedAccount ? formatMoney(q, selectedAccount.currency) : q.toString()}
                  onPress={() => setAmount(q.toString())}
                  variant={amount === q.toString() ? 'primary' : 'secondary'}
                  size="small"
